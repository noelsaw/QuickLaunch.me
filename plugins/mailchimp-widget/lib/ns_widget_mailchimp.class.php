...
class NS_Widget_MailChimp extends WP_Widget {
	...
	public function NS_Widget_MailChimp () {
		...
		$this->default_loader_graphic = get_bloginfo('template_url').'/plugins/mailchimp-widget/images/ajax-loader.gif';
	...
	public function add_scripts () {
		...
		wp_enqueue_script('ns-mc-widget', get_bloginfo('template_url') . '/plugins/mailchimp-widget/js/mailchimp-widget-min.js', array('jquery'), false);
	}
	...
	public function widget ($args, $instance) {
		...
		if ((isset($_COOKIE[$this->id_base . '-' . $this->number]) && $this->hash_mailing_list_id($this->number) == $_COOKIE[$this->id_base . '-' . $this->number]) || false == $this->ns_mc_plugin->get_mcapi()) {
			...
			echo '<div style="border:#C00;background-color:#FFEBE8;padding:0.6em;font-size:13px;">'.__('You\'ll need to enter the MailChimp API key in Settings > MailChip Widget in WP Admin panel, before using it. ', 'mailchimp-widget').'</div>';
			...
			if ($this->successful_signup) {
				...
				<form action="<?php echo $_SERVER['REQUEST_URI']; ?>" id="<?php echo $this->id_base . '_form-' . $this->number; ?>" method="post">
					...
					?>
						...
						<input id="<?php echo $this->id_base; ?>-email-<?php echo $this->number; ?>" type="text" name="<?php echo $this->id_base; ?>_email" class="email" placeholder="Signup for email newsletters" size="50" />
						<input class="btn" type="submit" name="<?php echo __($instance['signup_text'], 'mailchimp-widget'); ?>" value="<?php echo __($instance['signup_text'], 'mailchimp-widget'); ?>" />
					</form>
